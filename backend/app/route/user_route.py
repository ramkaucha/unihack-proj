from backend.app.controller.user_controller import user_controller
from flask import Blueprint, request, jsonify
from flask_jwt_extended import jwt_required, get_jwt_identity

user_bp = Blueprint('user_routes', __name__)
def userRoutes(app):
    @user_bp.route("/user/register", method=["POST"])
    def register():
        data = request.get_json()
        return user_controller.register(data)
    
    @user_bp.route("/user/login", method=["POST"])
    def login():
        data = request.get_json()
        return user_controller.login(data)
    
    @user_bp.route("/user/query_user_by_name", method=["GET"])
    @jwt_required
    ...
    @user_bp.route("/user/update_user_detail", method=["POST"])
    @jwt_required
    ...
    @user_bp.route("/user/delete_user", method=["POST"])
    @jwt_required
    def delete_user():
        current_user = get_jwt_identity()
        data = request.get_json()
    ...
    @user_bp.route("/user/get_user_name_ids", method=["GET"])
    @jwt_required
    ...
    @user_bp.route("/user/increase_credits", method=["POST"])
    def increase_credits():
        data = request.get_json()
        return user_controller.increse_credit(data)
    
    @user_bp.route("/user/query_credit_by_user", method=["GET"])
